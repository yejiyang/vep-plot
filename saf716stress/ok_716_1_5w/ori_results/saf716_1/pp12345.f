...
c      upper crust and lower crust ...
      ...
      dimension nleft(1000),nright(1000)
      dimension nleft0(1000),nright0(1000)
...
!      close(10)
!
!      open(40,file='slip.m',status='unknown',form='formatted')
!c multilayer slip output
!      do m=1,nlayer
!c    jump to the m layer
!      do i=1,nfpn
!        nleft(i) =nleft0(i) +np2*(nlayer-m)
!        nright(i)=nright0(i)+np2*(nlayer-m)
!      enddo
!
!c    calculate the fault slip and output it to the slip file
!      do i=1,nfpn
!      du=u0(1,nleft(i))-u0(1,nright(i))
!      dv=u0(2,nleft(i))-u0(2,nright(i))
!      nnleft   =  nleft(i) - np2*(nlayer-m)
!      nnright  =  nright(i)- np2*(nlayer-m)
!      x0=0.5*(coor(1,nnleft)+coor(1,nnright))
!      y0=0.5*(coor(2,nnleft)+coor(2,nnright))
!      duv=sqrt(du*du+dv*dv)
!      call xy2deg1(x0,y0,xdeg0,ydeg0)
!      write(40,1400) m,xdeg0,ydeg0,duv*1000.0/5.0
!      enddo
!      enddo
!      close(40)
!c**fault slip end
...
!      open(11,file='res_xy.txt',status='unknown',form='formatted')
!      do i=1,np2
!      k=np2*(nlayer-1)+i
...
c      pi==3.1415926==4*datan(1.d0)
      pi=4*datan(1.d0)/180.d0
      R=6378.e3

!! for deg to meter and rotate
!      xcent=-117.774
!      ycent=34.736
!      xleft=-122.0d0
!      xright=-112.0d0
...
c       add by jiyang
      ...
      lon=lon+step
      lat=ybottom-step
      do while ( lat .le. ytop ) 
      flag=-1.0
      lat=lat+step

!! for deg to meter and rotate
!c     projection
!      call trans2(R,lon,lat,xcent,ycent,c,d)
!      cx=c
!      cy=d
!c       write(66,1010) cx,cy
!c      xxx0=a11*cx+a12*cy-xmin+x0
!      xxx0=a11*cx+a12*cy
!c      yyy0=a21*cx+a22*cy-ymin+y0
!      yyy0=a21*cx+a22*cy
!      x3=xxx0
!      y3=yyy0      
...
c       write multi_layer results tegother, 
c       change nlayer-1 to nlayer-m; m=1,surface; m=nlayer, bottom
c      write(*,*) k
c
c..... u0
c
       ...
       zzd(1) = u0(1,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u0(1,nqudr(2,k)+np2*(nlayer-m))
       ...
       zzd(1) = u0(2,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u0(2,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u0(2,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u0(2,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u02= top*1000.d0

       zzd(1) = u0(3,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u0(3,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u0(3,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u0(3,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u03= top*1000.d0
...
c..... u1
c
       zzd(1) = u1(1,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u1(1,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u1(1,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u1(1,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u11= top
      
       zzd(1) = u1(2,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u1(2,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u1(2,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u1(2,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u12= top

       zzd(1) = u1(3,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u1(3,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u1(3,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u1(3,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u13= top

       zzd(1) = u1(4,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u1(4,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u1(4,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u1(4,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u14= top

       zzd(1) = u1(5,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u1(5,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u1(5,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u1(5,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u15= top

       zzd(1) = u1(6,nqudr(1,k)+np2*(nlayer-m))
       zzd(2) = u1(6,nqudr(2,k)+np2*(nlayer-m))
       zzd(3) = u1(6,nqudr(3,k)+np2*(nlayer-m))
       zzd(4) = u1(6,nqudr(4,k)+np2*(nlayer-m))
       call locate_qudr(xxd,yyd,xxd0,yyd0,rr,ss)
       call inter_quadri(rr,ss,zzd,top)
       u16= top

c       write(12,1000) m,lon,lat,uu01,uu02,uu03,u14,u15,u16
c       write multi_layer results tegother
...
c      write(*,*) ii,ress(ii),resd(ii),acos(0.5d0)
      ...
      write(12,1000) m,lon,lat,u01,u02,u03,u11,u12,u13,u14,u15,u16,
...
c       add by jiyang
      ...
      write(122,1001) lon,lat, m, (bb(j),j=1,6), idx,lon, lat
...
cjiyang need check
c%% %%%%%%%%%%%%%%% transfer the degree back to meter
      subroutine deg2xy1(xdeg,ydeg,x,y)
        implicit real*8 (a-h,l,o-z)
        long0=-117.774
        lat0=34.736
        pi=4*datan(1.d0)
c     transfer meter to degree
        x=111263*abs(cos((pi/180)*((lat0+ydeg)/2)))*(xdeg-long0)
        y=111263*(ydeg-lat0)
...
cjiyang       
c%% %%%%%%%%%%%%%%% transfer the degree to meter
c  %%%%%%%%% set a local center, using the center of the region as
clocal(0,0) 
c      x0=-117.774; y0=34.736; % local(0,0)
c      x=long; y=lat;
c      xn=111263*abs(cos((pi/180)*((y+y0)/2)))*(x-x0);
c      yn=111263*(y-y0);
        subroutine trans2(R,x,y,a,b,c,d)
        implicit real*8 (a-h,l,o-z)
        dimension p(3,3),q(3,3),s(3,3),xx(3),xxx(3),xy(3)
c       (a,b) = local(0,0)
c       pi==3.1415926==4*datan(1.d0)
        pi=4*datan(1.d0)/180.d0
        c=111263*abs(cos((pi)*((y+b)/2)))*(x-a)
        d=111263*(y-b)
        return
        end
