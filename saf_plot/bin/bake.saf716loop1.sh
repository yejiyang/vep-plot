...
grdimage data1.grd -C./cpt/vect_hui.cpt ${range} ${proj} -K -V -P -O >>${psfile}
#plot the faults
psclip ./para/saf627bddegree.txt $range $proj -m -O -K >>$psfile
#psxy fault_data.gmt $proj $range -W7/0/0/0 -m -O -V -K>>$psfile
psxy ./para/saf1.txt.gmt $proj $range -W7/0/0/0 -m -O -V -K>>$psfile
psclip -C -O -K >>$psfile
psxy ./para/saf627bddegree.txt $proj $range -W7/0/0/0 -m -O -V -K>>$psfile
...
psscale -D7.035c/7.3c/2.06c/0.18ch -C./cpt/vect_hui.cpt  -I0.3 -B10  -O -K -V>> ${psfile}
...
grdsample data.grd -Gdata1.grd -I0.01 -Qb
grdimage data1.grd -C./cpt/cfs_hui.cpt $proj -R -K -V -P -O >>$psfile
...
gmtset  ANNOT_FONT_SIZE 6p
psscale -D7.035c/7.3c/2.06c/0.18ch -C./cpt/cfs_hui.cpt  -S -B10  -O -K -V>> ${psfile}
...
xyz2grd mean.xyz -Gdata.grd -I0.1 -R
grdsample data.grd -Gdata1.grd -I0.01 -Qb
grdimage data1.grd -C./cpt/shear_hui.cpt $proj -R -K -V -P -O >>$psfile
...
gmtset  ANNOT_FONT_SIZE 6p
psscale -D7.035c/7.3c/2.06c/0.18ch -C./cpt/shear_hui.cpt   -B15 -S  -O -K -V>> ${psfile}
...
grdimage ./para/topo.grd -I./para/topo_gradient.grd  $proj $range -C./cpt/ca.cpt  -P -O -K -V >>$psfile
grdcontour ./para/topo.grd -C500 -R -J -O -V -K >> $psfile
psscale -C./cpt/ca.cpt -D4/-1.2/6/0.5h -B2000/:meters: -O -K >>$psfile
psxy ./para/fault_data.gmt -m $proj $range -W2 -O -V -K >> $psfile
