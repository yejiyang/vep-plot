...
c
     ...
     & idisp1,idisp2
      write(41) lgio,t0,tmax,dt,num0,knode
c
      knb1=kdgof*knode*1
      ...
      knb17=num0*1
      if (knb17/2*2 .lt. knb17) knb17=knb17+1
      kna0=1
      kna1=kna1+kna0
      kna2=kna2+kna1
      kna3=kna3+kna2
     ...
     *filename,kelemsub,ia(knb15),nne,ia(knb16))
c
1000    CONTINUE
      close(30)
      close(31)
     ...
     *nmta,
     *filename,kelemsub,nodvar1,nne,ielem)
      implicit real*8 (a-h,o-z)
...
cc      if((nne.eq.8).and.(ncoor.eq.3)) nelemtype=4
...
c
      if((nne.eq.6).and.(ncoor.eq.3)) then
      open(39,file='lwt.z',form='formatted',status='unknown')
      read(39,*) nlayers
      close(39)
      numtri = num/nlayers
      numnodtri = knode/(nlayers+1)
      end if

cc      if((nne.eq.8).and.(ncoor.eq.3)) then
cc      open(39,file='lwt.z',form='formatted',status='unknown')
cc      read(39,*) nlayers
cc      close(39)
cc      numtri=num/nlayers
cc      numnodtri = knode/(nlayers+1)
cc      end if
c 
C...... OPEN ELEMENT FILE FOR DATA PARTITIONING
      ...
      else if((nne.eq.6).and.(ncoor.eq.3)) then
      WRITE(10,*) numtri,nelemtype
      do i=1,numtri
      write(10,8001) (NODE((I-1)*NNODE+J),J=1,3)
      end do
cc      else if((nne.eq.8).and.(ncoor.eq.3)) then
cc      WRITE(10,*) numtri,nelemtype
cc      do i=1,numtri
cc      write(10,8001) NODE((I-1)*NNODE+1),NODE((I-1)*NNODE+2),
cc     *NODE((I-1)*NNODE+3),NODE((I-1)*NNODE+4)
cc      end do
      else
      WRITE(10,*) NUM,nelemtype
      do i=1,num
...
c       Chech the partition data
       if((nne.eq.6).and.(ncoor.eq.3)) then
       if(ne.ne.numtri) print *, 'number of surface element error',
     &   ne,numtri,num,nlayers
       if(np.ne.numnodtri) print *, 'number of surface node error',
     &   np,numnodtri,knode,numnod,nlayers
       do j = 2,nlayers
       do i = 1,numtri
       idelem((j-1)*numtri+i) = idelem(i)
       end do
       end do
c
       do j = 2,nlayers+1
       do i = 1,numnodtri
       idnode((j-1)*numnodtri+i) = idnode(i)
       end do
       end do 
cc       else if((nne.eq.8).and.(ncoor.eq.3)) then
cc       if(ne.ne.numtri) print *, 'number of surface element error',
cc     &   ne,numtri,num,nlayers
cc       if(np.ne.numnodtri) print *, 'number of surface node error',
cc     &   np,numnodtri,knode,numnod,nlayers
cc       do j = 2,nlayers
cc       do i = 1,numtri
cc       idelem((j-1)*numtri+i) = idelem(i)
cc       end do
cc       end do
c
cc       do j = 2,nlayers+1
cc       do i = 1,numnodtri
cc       idnode((j-1)*numnodtri+i) = idnode(i)
cc       end do
cc       end do
...
c...... OPEN ELEMENT FILE FOR DATA PARTITIONING
     ...
     + ' Elemtype ','Hexahedra',' Nnode',8
        else if((nne.eq.8).and.(ncoor.eq.3)) then
        write(10,*) ' Mesh "Whole" Dimension',ncoor,
     + ' Elemtype ','Hexahedra',' Nnode',8
        else
        write(10,*) ' Mesh "Whole" Dimension',ncoor,
     +' Elemtype ',filename1(20),' Nnode',nnode-1
      ...
      if((nne.eq.10).and.(ncoor.eq.3)) then
        ...
        do i = 1,num
        write(10,1100) i,NODE((I-1)*NNODE+1),NODE((I-1)*NNODE+2),
     +                 NODE((I-1)*NNODE+3),NODE((I-1)*NNODE+3),
     +                 NODE((I-1)*NNODE+4),NODE((I-1)*NNODE+5),
     +                 NODE((I-1)*NNODE+6),NODE((I-1)*NNODE+6),
     +                 idelem(i)
        end do
cc      else  if((nne.eq.8).and.(ncoor.eq.3)) then
cc        do i = 1,num
cc        write(10,1100) i,NODE((I-1)*NNODE+1),NODE((I-1)*NNODE+2),
cc     +                 NODE((I-1)*NNODE+3),NODE((I-1)*NNODE+4),
cc     +                 NODE((I-1)*NNODE+5),NODE((I-1)*NNODE+6),
cc     +                 NODE((I-1)*NNODE+7),NODE((I-1)*NNODE+8),
cc     +                 idelem(i)
cc        end do
      else
        do i=1,num
...
c       do i=1,nod_iblk
c       print *, idnode1(inode(i)),inode(i)
c       end do
c       stop
...
c
c      
       print *,'nod_iblk,ncoor,num_iblk,nnode,nne,knode=='
       print *,nod_iblk,ncoor,num_iblk,nnode,nne,knode
       if(iprint.eq.1) then
c
c
     ...
     + NODEI((I-1)*NNODE+11)
        ...
        write(10,1100) i,jnode(NODEI((I-1)*NNODE+1)),
     +                   jnode(NODEI((I-1)*NNODE+2)),
     +                   jnode(NODEI((I-1)*NNODE+3)),
     +                   jnode(NODEI((I-1)*NNODE+3)),
     +                   jnode(NODEI((I-1)*NNODE+4)),
     +                   jnode(NODEI((I-1)*NNODE+5)),
     +                   jnode(NODEI((I-1)*NNODE+6)),
     +                   jnode(NODEI((I-1)*NNODE+6)),
     +                   NODEI(I*NNODE)
cc       else if((nne.eq.8).and.(ncoor.eq.3)) then
cc        write(10,1100) i,jnode(NODEI((I-1)*NNODE+1)),
cc     +                   jnode(NODEI((I-1)*NNODE+2)),
cc     +                   jnode(NODEI((I-1)*NNODE+3)),
cc     +                   jnode(NODEI((I-1)*NNODE+4)),
cc     +                   jnode(NODEI((I-1)*NNODE+5)),
cc     +                   jnode(NODEI((I-1)*NNODE+6)),
cc     +                   jnode(NODEI((I-1)*NNODE+7)),
cc     +                   jnode(NODEI((I-1)*NNODE+8)),
cc     +                   NODEI(I*NNODE)
      else
...
c      WRITE(*,*) 'NODE ='
c      do i = 1,num
c      WRITE(*,*) (NODE((I-1)*NNODE+J),J=1,NNODE)
c      end do
...
c
      do i=1,MMATE
      do j=1,NMATE
      EMATEall(matall+(I-1)*NMATE+J)=EMATE((I-1)*NMATE+J)
      end do
      end do
      matall = matall + mmate*nmate
      mmta(itype) = mmate
      nmta(itype) = nmate
      print *,'partition:iblk,nblk,nnode,nodall matall=',
     * iblk,nblk,nnode, nodall,matall
      end do
C     end do of itype
      iidet = 1
C...  Write etype0 file
      write(29) numtyp,nodall,matall
      write(29) (iidet,numa(i),nnea(i),mmta(i),nmta(i),i=1,numtyp),
     &nod_iblk,(inode(i),i=1,nod_iblk)
      write(29) numa(1),(ielem(i),i=1,numa(1))
C ... Write elem0 file
...
c      WRITE(*,*) 'NUM =',NUM,' NNODE =',NNODE
c      WRITE(*,*) 'NODE ='
c      WRITE(*,*) ((NODE((I-1)*NNODE+J),J=1,NNODE),I=1,NUM)
      READ (30) MMATE,NMATE,((EMATE((I-1)*NMATE+J),J=1,NMATE),
     *I=1,MMATE)
c      WRITE(*,*) 'MMATE =',MMATE,' NMATE =',NMATE
c      WRITE (*,*) 'EMATE ='
